...
class PinForm(FlaskForm):
    ...
    description = TextAreaField('Description', default='', validators=[Length(min=0, max=500, message='Description must be less than 500 characters.')])
    image = FileField('image', validators=[DataRequired('Please provide an image.')])
    link = StringField('Link', default='', validators=[Length(min=0, max=500, message='Link must be less than 500 characters.'), valid_link])
